...
$objResult = mysql_fetch_object($query);
$inshtml="<li><a href='edit.php?id=1' title='".$objResult->name."'><img src='".$objResult->thumbnail."' alt='".$objResult->name."'>".$objResult->name."</a></li>";
$c=2;
$query = mysql_query("SELECT name, url, caption, thumbnail, hello, category FROM content WHERE id='$c'");
$objResult = mysql_fetch_object($query);
do {
	$inshtml=$inshtml."<li><a href='edit.php?id=".$c."' title='".$objResult->name."'><img src='".$objResult->thumbnail."' alt='".$objResult->name."'>".$objResult->name."</a></li>";
	$c=$c+1;
	$query = mysql_query("SELECT name, url, caption, thumbnail, hello, category FROM content WHERE id='$c'");
	$objResult = mysql_fetch_object($query);
